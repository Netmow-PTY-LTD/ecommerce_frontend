...
export default function EditProductPage() {
  ...
  const [filteredProducts, setFilteredProducts] = useState<SimilarProduct[]>([]);
  const [selectedSimilarProducts, setSelectedSimilarProducts] = useState<SimilarProduct[]>([]);
  ...
  // Fetch product after gallery images are loaded
  useEffect(() => {
    if (isAuthenticated && galleryImagesLoaded) {
      fetchProduct();
    }
  ...
  // Load similar products after product and allProducts are loaded
  useEffect(() => {
    if (product && product.similar_products && product.similar_products.length > 0 && allProducts.length > 0) {
      const similarProductsIds = product.similar_products;
      const similarProductsData = allProducts.filter((p: SimilarProduct) =>
        similarProductsIds.includes(p.id)
      );
      setSelectedSimilarProducts(similarProductsData);
    }
  }, [product, allProducts]);
  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData({
        ...
        purchase_tax: productData.purchase_tax?.toString() || '',
        sales_tax: productData.sales_tax?.toString() || '',
        stock_quantity: productData.stock_quantity?.toString() || '',
        initial_stock: productData.initial_stock?.toString() || '',
      ...
      if (productData.image_url) {
        const mainImage = galleryImages.find(img => img.url === productData.image_url);
        if (mainImage) {
          setSelectedMainImage(mainImage);
        } else {
          // Image exists in DB but not in gallery - create a fallback object
          const filename = productData.image_url.split('/').pop() || 'image';
      ...
      // Set selected thumbnail if exists
      if (productData.thumb_url) {
        const thumbImage = galleryImages.find(img => img.url === productData.thumb_url);
        if (thumbImage) {
          setSelectedThumbnail(thumbImage);
        } else {
          // Thumbnail exists in DB but not in gallery - create a fallback object
          const filename = productData.thumb_url.split('/').pop() || 'thumbnail';
          setSelectedThumbnail({
            id: Date.now() + 1,
            filename: filename,
            originalName: filename,
            url: productData.thumb_url,
            size: 0,
            category: 'products'
          });
        }
      ...
      // Set selected gallery images if exist
      if (productData.gallery_items && productData.gallery_items.length > 0) {
        const galleryImagesData = galleryImages.filter(img => productData.gallery_items.includes(img.url));
        setSelectedGalleryImages(galleryImagesData);
      }
  ...
  const fetchGalleryImages = async () => {
    try {
      ...
      setGalleryImagesLoaded(true); // Set to true even on error so product can load
    }
  };
  ...
  const fetchProducts = async () => {
    try {
      ...
      setAllProducts(products.filter((p: SimilarProduct) => p.id !== parseInt(productId)));
    } catch (error) {
  ...
  const handleToggleSimilarProduct = (product: SimilarProduct) => {
              ...
              <div>
                ...
                {!formData.image_url && (
                  <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center bg-slate-50">
                    ...
                    <p className="text-xs text-slate-400">Select from gallery or paste a URL above</p>
                  </div>
                )}
              </div>
