...
export default function OrderDetailPage() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      // Use total_amount from API if items are empty, otherwise calculate
      let calculatedTotal = itemsSubtotal + taxAmount + shippingCost - discountAmount;
      if (data.items && data.items.length === 0) {
        calculatedTotal = parseFloat(data.total_amount || 0);
      }
      ...
      const transformedOrder: Order = {
        ...
        customer_name: data.customer?.name && data.customer?.name !== 'undefined undefined'
          ? data.customer.name
          : [data.customer?.first_name, data.customer?.last_name].filter(Boolean).join(' ') || 'N/A',
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
              ...
              {order.items.length === 0 ? (
                <div className="p-6 text-center bg-slate-50">
                  ...
                  <p className="mt-2 text-sm text-slate-600">No items found for this order</p>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-3 py-1 text-xs font-semibold rounded-full border-2 ${getPaymentStatusColor(order.payment_status)}`}>
